...
unsigned int TextureFromFile(const char *path, const string &directory, bool gamma = true)
{
	...
	if (unsigned char *data = SOIL_load_image(filename.c_str(), &width, &height, &nrComponents, 0))
	{
		GLenum format;
		if (nrComponents == 1)
			format = GL_RED;
...
std::vector<Texture> Model::loadMaterialTextures(const aiMaterial *mat, const aiTextureType type, string typeName)
{
	std::vector<Texture> textures;
